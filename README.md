...
14 articles were re-fetched verbatim (`13, 14, 16, 19, 21, 32, 36, 51A, 55, 110, 154, 352, 368, 370`); the remaining 44 are tagged `curated_not_verified`. Only `fetched_from_wikisource` chunks may serve as the `source_chunk_id` for `generate_mcq` correct answers.
...
Uses Gemini via `langchain-google-genai` for real tutoring; falls back to a no-API-key model so the CLI works without credentials.
...
| Bare Constitution articles | 58 articles | 14 verbatim from Wikisource, 44 curated; all provenance-tagged |
